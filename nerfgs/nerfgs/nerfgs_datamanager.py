...
@dataclass
class NerfgsDatamanagerConfig(DataManagerConfig):
    _target: Type = field(default_factory=lambda: NerfgsDatamanager)
...
class NerfgsDatamanager(DataManager, Generic[TDataset]):
    """
    ...
    config: NerfgsDatamanagerConfig
    ...
    def dataset_type(self) -> Type[TDataset]:
        ...
        orig_class: Type[NerfgsDatamanager] = get_orig_class(self, default=None)  # type: ignore
        if type(self) is NerfgsDatamanager and orig_class is None:
            return default
        if orig_class is not None and get_origin(orig_class) is NerfgsDatamanager:
            return get_args(orig_class)[0]

        # For inherited classes, we need to find the correct type to instantiate
        for base in getattr(self, "__orig_bases__", []):
            if get_origin(base) is NerfgsDatamanager:
    ...
    @property
    def fixed_indices_train_dataloader(self) -> List[Tuple[Cameras, Dict]]:
        """
        Pretends to be the dataloader for training data, it returns a list of (camera, data) tuples
        """
        image_indices = [i for i in range(len(self.train_dataset))]
        data = deepcopy(self.cached_train)
