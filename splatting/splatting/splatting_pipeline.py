...
from splatting.splatting_model import SplattingModelConfig, SplattingModel
...
@dataclass
class splattingPipelineConfig(VanillaPipelineConfig):
    """Configuration for pipeline instantiation"""

    _target: Type = field(default_factory=lambda: splattingPipeline)
    """target class to instantiate"""
    # datamanager: DataManagerConfig = field(default_factory=DataManagerConfig)
    """specifies the datamanager config"""
    model: ModelConfig = field(default_factory=SplattingModelConfig)
    """specifies the model config"""
    export_nerf_gs_data: bool = False

class splattingPipeline(VanillaPipeline):
  def __init__(
    self,
    config: splattingPipelineConfig,
    device: str,
    ...
    grad_scaler: Optional[GradScaler] = None,
      ...
      if world_size > 1:
          self._model = typing.cast(
              SplattingModel, DDP(self._model, device_ids=[local_rank], find_unused_parameters=True)
          )
